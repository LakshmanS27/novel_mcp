# Evaluation Harness — SecDev 2026
...
With `DLP_MCP_LLM_ENABLED=true`:

- System B switches to the `mcp_client` orchestration path. In that mode, the
  LLM uses the MCP server tools over MCP to perform selective candidate
  discovery and targeted scanning.
- System B2 remains a direct `mcp_server.core` pipeline, but it now runs with
  server-side LLM semantic inference and explanation enabled through
  `run_full_analysis()`.

To run with LLM-assisted selective orchestration:
```bash
DLP_MCP_LLM_ENABLED=true python -m eval.run_evaluation /path/to/repo
```

Requirements for LLM-enabled mode:

- at least one valid provider key: `COMMANDCODE_API_KEY`, `OPENAI_API_KEY`, or `OPENROUTER_API_KEY`
- if you set a custom provider base URL, use the API root such as `.../v1` or `.../provider/v1`, not a full `.../chat/completions` endpoint
- local MCP server will be auto-started if not already running
- server-side B2 semantic inference falls back to the same auto-detected provider/model unless `DLP_MCP_LLM_*` overrides are explicitly set
