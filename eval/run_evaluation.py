...
async def run_selective_via_mcp_client(target: Path, settings: Settings) -> dict[str, Any]:
    """Run selective evaluation through the MCP client and LLM orchestration."""
    from mcp_client.eval_agent import run_eval_via_mcp

    start = time.perf_counter()
    result = await run_eval_via_mcp(str(target))
    elapsed = time.perf_counter() - start
    result["mode"] = "selective"
    result["execution_path"] = "mcp_client_llm"
...
async def run_graph_pipeline(target: Path, settings: Settings) -> dict[str, Any]:
    ...
    return {
        "mode": "graph_pipeline",
        "execution_path": "direct_core_llm" if settings.llm_enabled else "direct_core",
...
async def main(target_path: str, skip_seed: bool = False, skip_baseline: bool = False) -> None:
    ...
    # Default to deterministic evaluation unless explicitly enabled.
    os.environ.setdefault("DLP_MCP_LLM_ENABLED", "false")
    settings = Settings()
    mcp_server_process: asyncio.subprocess.Process | None = None
    ...
    try:
        ...
        if settings.llm_enabled:
            from mcp_client.eval_agent import ensure_local_mcp_server

            print("\n[System B] Running candidate-selector pipeline via MCP client + LLM...")
            mcp_server_process = await ensure_local_mcp_server()
            selective_result = await run_selective_via_mcp_client(target, settings)
        else:
        ...
        if settings.llm_enabled:
            print("\n[System B2] Running graph-based pipeline with server-side LLM semantic inference...")
        else:
